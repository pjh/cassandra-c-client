/* cassandra_client.c - Implementation of basic Cassandra C Client.
 *
...
{
	if (*err) {
		//ccc_error("Got GError message: %s\n", (*err)->message);
		g_error_free(*err);
		*err = NULL;
	}
	//else {
...
int convert_bytearray_to_string(GByteArray *bytes, char **string)
{
	...
	*string = malloc(len+1);
	...
	(*string)[len] = '\0';
	//ccc_debug("converted byte array to string: %s\n", *string);
...
cassandra_node *cassandra_node_alloc(const char *hostname, unsigned int port)
{
	...
	if (node == NULL) {
		ccc_error("malloc(cassandra_node) failed\n");
		return NULL;
	}
	...
	if (node->hostname == NULL) {
		ccc_error("malloc(hostname) failed\n");
		free(node);
		return NULL;
	}
...
int cassandra_describe_keyspaces(cassandra_node *node)
{
	...
	if (invalidreq_except) {
		ccc_error("cassandra_if_describe_keyspaces() returned "
	...
	if (retbool != TRUE) {
		ccc_error("cassandra_if_describe_keyspaces() returned error\n");
	...
	for (i = 0; i < retarray->len; i++) {
		...
		if (ksdef) {
			ccc_debug("Keyspace name=[%s]\n", ksdef->name);
		} else {
			ccc_debug("Keyspace pointer is null!?\n");
		}
	}
	g_ptr_array_free(retarray, TRUE);
...
int cassandra_drop_column_family(cassandra_node *node, const char *col_fam_name)
{
	...
	if (invalidreq_except) {
		if (strncmp(invalidreq_except->why, not_found_message,
			...
			ccc_debug("did not find column to drop: %s\n", col_fam_name);
			ccc_debug("returning -2\n");
			retval = -2;
		} else {
			ccc_error("cassandra_if_system_drop_column_family(%s) returned "
			    "InvalidRequestException: %s\n", col_fam_name,
			    invalidreq_except->why);
		}
		g_object_unref(invalidreq_except);
	...
	ccc_debug("cassandra_if_system_drop_column_family(%s) succeeded, "
		...
		char *col_fam_name)
{
	...
	if (cf_def == NULL) {
		ccc_error("g_object_new(TYPE_CF_DEF) failed\n");
		return -1;
	}
	...
	if (invalidreq_except) {
		 ...
		 */
		ccc_error("cassandra_if_system_add_column_family(%s) returned "
		    "InvalidRequestException: %s\n", col_fam_name,
		    invalidreq_except->why);
		g_object_unref(invalidreq_except);
		retbool = FALSE;  //make sure to hit error case that comes next
	...
	if (retbool != TRUE) {
		ccc_error("cassandra_if_system_add_column_family(%s) returned error\n",
		    col_fam_name);
		free(retstring);
	 ...
	 */
	ccc_debug("cassandra_if_system_add_column_family(%s) succeeded, "
	    "returned new schema version ID=%s\n",
	    col_fam_name, retstring);
	free(retstring);
	g_object_unref(cf_def);
...
int cassandra_start(cassandra_node **node, unsigned int port)
{
	...
	*node = cassandra_node_alloc(CASSANDRA_HOSTNAME, port);
	if (*node == NULL) {
		ccc_error("cassandra_node_alloc() failed\n");
		return -1;
	}
	 ...
	 */
	ccc_debug("Attempting to connect to server %s:%u\n",
	...
	if ((*node)->tsocket == NULL) {
		ccc_error("g_object_new(THRIFT_TYPE_SOCKET) failed\n");
	...
	(*node)->tframedtransport =
	    (ThriftFramedTransport *)g_object_new(THRIFT_TYPE_FRAMED_TRANSPORT,
	...
	if ((*node)->tframedtransport == NULL) {
		ccc_error("g_object_new(THRIFT_TYPE_FRAMED_TRANSPORT) failed\n");
		cassandra_node_free(*node);
		return -1;
	}
	...
	(*node)->tbinaryprotocol =
	    (ThriftBinaryProtocol *)g_object_new(THRIFT_TYPE_BINARY_PROTOCOL,
	...
	if ((*node)->tbinaryprotocol == NULL) {
		ccc_error("g_object_new(THRIFT_TYPE_BINARY_PROTOCOL) failed\n");
		cassandra_node_free(*node);
		return -1;
	}
	...
	if ((*node)->client == NULL) {
		ccc_error("g_object_new(TYPE_CASSANDRA_CLIENT) failed\n");
		cassandra_node_free(*node);
		return -1;
	}
	...
	if (!retbool) {
		ccc_error("thrift_transport_open() failed; probably means that "
	...
	check_gerror(&err);
	if (!retbool) {
		ccc_error("cassandra_if_describe_version() failed; connection is "
		    "open, but something else is wrong\n");
		cassandra_node_free(*node);
		return -1;
	}
	ccc_print("Successfully connected to Cassandra node at %s:%u, has "
	...
	if (ret != 0) {
		ccc_error("cassandra_set_keyspace(%s) returned error=%d\n",
		    CASSANDRA_KEYSPACE, ret);
		return -1;
	}
	ccc_print("set keyspace to %s\n", CASSANDRA_KEYSPACE);

	ret = cassandra_drop_column_family(*node, CASSANDRA_COL_FAM_NAME);
	if (ret != 0 && ret != -2) {
		ccc_error("cassandra_drop_column_family(%s) returned error=%d\n",
	...
	if (ret != 0) {
		ccc_error("cassandra_add_column_family(%s) returned error=%d\n",
		    CASSANDRA_COL_FAM_NAME, ret);
		return -1;
	}
	ccc_print("Added empty column family %s to keyspace\n",
	    CASSANDRA_COL_FAM_NAME);
	 ...
	 * evaluation purposes.
	 */
	ccc_print("Sleeping for a bit to allow column family changes to settle\n");
	sleep(2);
...
int cassandra_noop1(cassandra_node *node)
{
	...
	if (!retbool) {
		ccc_error("cassandra_if_describe_version() failed\n");
		return -1;
	}
...
int cassandra_noop2(cassandra_node *node)
{
	...
	if (!retbool) {
		ccc_error("cassandra_if_describe_cluster_name() failed\n");
		return -1;
	}
	/* Ignore everything that's returned; this function does as little as
	 * possible, to be as much of a noop as possible.
...
int cassandra_noop3(cassandra_node *node)
{
	...
	if (!retbool) {
		ccc_error("cassandra_if_describe_partitioner() failed\n");
		return -1;
	}
	/* Ignore everything that's returned; this function does as little as
	 * possible, to be as much of a noop as possible.
...
int cassandra_noop4(cassandra_node *node)
{
	...
	if (!retbool) {
		ccc_error("cassandra_if_describe_snitch() failed\n");
		return -1;
	}
	/* Ignore everything that's returned; this function does as little as
	 * possible, to be as much of a noop as possible.
...
int cassandra_noop5(cassandra_node *node)
{
	...
	if (!retbool) {
		ccc_error("cassandra_if_describe_keyspaces() failed\n");
		return -1;
	}
	/* Ignore everything that's returned; this function does as little as
	 * possible, to be as much of a noop as possible.
...
int cassandra_worker_stop(cassandra_node *node)
{
	...
	if (retbool != TRUE) {
		ccc_error("thrift_transport_close() returned error!\n");
		retval = -1;
	}
	ccc_debug("thrift_transport_close() succeeded, disconnected from "
	    "Cassandra node\n");

	/* Free the cassandra_node struct and all associated GObjects: */
	cassandra_node_free(node);
	ccc_debug("freed cassandra_node struct\n");
...
int cassandra_put(cassandra_node *node, const char *key, const char *value)
{
	 ...
	 * choices (see http://maxgrinev.com/2010/07/09/a-quick-introduction-to-the-cassandra-data-model/):
	 *   Single keyspace
	...
	if (col_parent == NULL) {
		ccc_error("g_object_new(TYPE_COLUMN_PARENT) returned error\n");
		return -1;
	}
	...
	check_gerror(&err);
	if (invalidreq_except) {
		ccc_error("cassandra_if_insert(%s, %s) returned "
		    "InvalidRequestException: %s\n", key, value,
		    invalidreq_except->why);
		g_object_unref(invalidreq_except);
		retbool = FALSE;  //make sure to hit error case that comes later
	...
	if (retbool != TRUE) {
		ccc_error("cassandra_if_insert(%s, %s) returned error\n", key, value);
		return -1;
	}
	ccc_debug("Successfully put(%s, %s) in column family %s\n", key, value,
	    col_fam_name);
...
int cassandra_put_noop(cassandra_node *node, const char *key, const char *value)
{
	 ...
	 */
	ccc_print("col_fam_name=%s; key_bytearray[0]=%c; column name[0]=%c; "
	    "column value[0]=%c; timestamp=%lld\n",
	    col_parent->column_family,
	    (key_bytearray->data)[4],
	    (column->name->data)[4],
	    (column->value->data)[4],
	    (long long int)column->timestamp);
	retbool = cassandra_if_describe_cluster_name(node->iface, &retstringptr, &err);
	check_gerror(&err);
	if (invalidreq_except) {
		ccc_error("cassandra_if_insert(%s, %s) returned "
...
int cassandra_get(cassandra_node *node, const char *key, char **value)
{
	...
	if (column_path == NULL) {
		ccc_error("g_object_new(TYPE_COLUMN_PATH) failed\n");
		g_byte_array_free(key_bytearray, TRUE);
		return -1;
	}
	...
	retval = -1;
	if (invalidreq_except) {
		ccc_error("cassandra_if_get(%s) returned InvalidRequestException: "
		    "%s\n", key, invalidreq_except->why);
		g_object_unref(invalidreq_except);
		retbool = FALSE;  //make sure to hit error case that comes later
	}
	if (unavail_except) {
		ccc_error("cassandra_if_get(%s) returned UnavailableException\n", key);
		g_object_unref(unavail_except);
		retbool = FALSE;  //make sure to hit error case that comes later
	}
	if (timedout_except) {
		ccc_error("cassandra_if_get(%s) returned TimedOutException\n", key);
		g_object_unref(timedout_except);
		retbool = FALSE;  //make sure to hit error case that comes later
	}
	if (notfound_except) {
		//ccc_error("cassandra_if_get(%s) returned NotFoundException\n", key);
	...
	if (col_or_super_col) {
		if (col_or_super_col->__isset_column) {
			...
			ret = convert_bytearray_to_string(col_or_super_col->column->name,
					&get_name);
			if (ret != 0) {
				ccc_error("convert_bytearray_to_string(name) returned "
				    "error=%d\n", ret);
				retbool = FALSE;
				get_name = NULL;
			} else {
				ccc_debug("got column name=%s\n", get_name);
			}
			ret = convert_bytearray_to_string(col_or_super_col->column->value,
					&get_value);
			if (ret != 0) {
				ccc_error("convert_bytearray_to_string(value) returned "
				    "error=%d\n", ret);
				retbool = FALSE;
				get_value = NULL;
			} else {
				ccc_debug("got column value=%s\n", get_value);
			}
			...
			case DM_ROWKEYS_USECOLVALS:
				...
				*value = get_value;
				ccc_debug("got column name=%s, expect it to match single "
			...
			case DM_ROWKEYS_USECOLNAMES:
				...
				*value = get_name;
				...
				ccc_warn("got column value=%s, expect it to be NULL "
				    "(right? ? ?)\n", get_value);
				if (get_value) {
					free(get_value);
				}
			...
			case DM_COLNAMEKEYS:
				/* value that we return comes from the column's value: */
				*value = get_value;
				ccc_debug("got column name=%s, expect it to match key=%s\n",
				    get_name, key);
				if (get_name) {
					free(get_name);
				}
		...
		if (col_or_super_col->__isset_super_column) {
			ccc_error("unexpectedly got a SuperColumn!\n");
			retbool = FALSE;
		}
		if (!col_or_super_col->__isset_column && !col_or_super_col->__isset_super_column) {
			ccc_error("got neither a column nor a super column, this should "
			    "never happen\n");
			retbool = FALSE;
		}
	...
	if (retbool != TRUE) {
		//ccc_error("cassandra_if_get(%s) returned error (or "
		//          "convert_bytearray_to_string() failed)\n", key);
		*value = NULL;
		return retval;
	}
	ccc_debug("Successful get(%s)=%s\n", key, *value);
